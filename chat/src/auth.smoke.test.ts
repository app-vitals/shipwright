...
function fakeAdminTokenService(): Pick<ChatTokenServiceLike, "validate"> {
  return {
...
function fakeAgentTokenService(): Pick<ChatTokenServiceLike, "validate"> {
  return {
    async validate(raw: string) {
...
function makeAuthApp(
  tokenService: Pick<ChatTokenServiceLike, "validate">,
