 ...
 * native Istanbul JSON format) against hand-built fixtures, plus the pure
 * filtering/aggregation/failure-computation logic extracted out of main()
 * (MTC-1.7): the file itself measures coverage but was, until this refactor,
 * not meaningfully covered by it. Nothing here touches the filesystem or
 * triggers the script's process.exit side effects — main()'s thin I/O/CLI
 * wiring is intentionally left uncovered, consistent with this repo's
