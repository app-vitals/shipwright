...
- **CI/workflow outbound triggers**: removing a workflow step that emits a cross-system signal — a dispatch event, a webhook call, a notification to an external service — is a breaking change if any downstream system depends on it. These consumers live in other repos or services and won't appear in a local grep; they'll simply stop firing. Before removing any outbound dispatch or notification step, confirm what is listening for it.
