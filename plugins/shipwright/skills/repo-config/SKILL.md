...
- **Required status checks** — every layer-relevant CI job from `test.yml` (lint, per-service unit, per-service integration, smoke, E2E). Keep this even for `--admin`-merging repos: it's still a real signal on the PR and still gates any merge attempt that doesn't go through `deploy.md` (e.g. a human merging via the GitHub UI).
- **Everything else that gates merging — up-to-date branches, conversation resolution, required approving reviews, enforce-on-admins — depends on who actually merges PRs in this repo.** Check whether the target repo has `shipwright-deploy` enabled (the "Stage 3, fully autonomous" ramp-up stage — see the `configuring-autonomy` docs page) before recommending values:
  - **A human merges PRs (`shipwright-deploy` off).** Require branches up to date: yes. Require conversation resolution: yes. Required approving reviews: ≥1 (≥2 for repos with multiple maintainers). Enforce on admins: yes, with a documented break-glass procedure. A human genuinely goes through this path, so all four are real gates.
  - **The agent merges its own PRs (`shipwright-deploy` on).** Require branches up to date: **no**. Require conversation resolution: **no**. Required approving reviews: **0**. Enforce on admins: **no**. `deploy.md` Step 4b merges every PR with `gh pr merge --admin`, which bypasses branch protection wholesale — not just the approval rule, but up-to-date and conversation-resolution too. None of these four settings gate the actual merge path in this repo; requiring them only adds friction for the rare human-initiated merge (e.g. forcing a rebase every time main advances) without adding real enforcement. GitHub also rejects self-APPROVE via the API, so `review.md` Step 10 force-downgrades an agent's review of its own PR to `COMMENT` with a `Verdict: APPROVE` marker — a required-review count can never be satisfied by the pipeline itself. For these repos the real gates are required status checks (above) plus canary/staged promotion after merge; don't recommend merge-gating settings the pipeline structurally can't satisfy or that `--admin` bypasses anyway.
...
- **Recommending required-review / enforce-admins / up-to-date / conversation-resolution as a blanket default regardless of who merges.** For a `shipwright-deploy`-enabled repo, `--admin` merges are the deploy model itself, not an emergency exemption (see Branch protection above), and it bypasses all four of these settings wholesale. Requiring ≥1 approval there doesn't add a review — self-APPROVE is blocked by GitHub's API — and requiring up-to-date/conversation-resolution just adds friction (forced rebases, stuck reviews) without gating the actual merge path. Stricter-looking, not actually stricter. Match the recommendation to the repo's actual merge path.
