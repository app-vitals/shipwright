...
- **Required PR** — no direct pushes. This is not bypassable by `--admin` (it's about whether a PR exists at all, not a merge-time rule) and applies regardless of who merges.
- **Required status checks** — every layer-relevant CI job from `test.yml` (lint, per-service unit, per-service integration, smoke, E2E). Keep this even for `--admin`-merging repos: it's still a real signal on the PR and still gates any merge attempt that doesn't go through `deploy.md` (e.g. a human merging via the GitHub UI). This is the actual gate for both stages below — CI must be green on the branch that lands, whether "up to date" is required or not.
- **Require conversation resolution before merging** — **yes, regardless of stage.** Unlike the settings below, this one has no cost: it's a metadata flag on review threads, not a CI job, so it burns zero Action minutes to enable. It doesn't block `deploy.md`'s `--admin` merge path either (same bypass as everything else in this section), so it adds no friction to the automated pipeline — it's a free safety net for any manual/human merge attempt. There's no reason tied to who-merges to turn this off.
- **Require branches to be up to date before merging, required approving reviews, and enforce-on-admins — depend on who actually merges PRs in this repo, and "up to date" specifically has a real cost with multiple agents in flight.** Check whether the target repo has `shipwright-deploy` enabled (the "Stage 3, fully autonomous" ramp-up stage — see the `configuring-autonomy` docs page) before recommending values:
  - **A human merges PRs (`shipwright-deploy` off).** Require branches up to date: yes. Required approving reviews: ≥1 (≥2 for repos with multiple maintainers). Enforce on admins: yes, with a documented break-glass procedure. A human genuinely goes through this path, so all three are real gates, and PR volume is low enough that "up to date" rebases aren't a meaningful cost.
  - **The agent merges its own PRs (`shipwright-deploy` on).** Require branches up to date: **no**. Required approving reviews: **0**. Enforce on admins: **no**. `deploy.md` Step 4b merges every PR with `gh pr merge --admin`, which bypasses branch protection wholesale, so none of these three settings gate the actual merge path — required status checks (above) plus canary/staged promotion after merge are the real gates. "Up to date" specifically also has a concrete, independent cost in this environment: GitHub's strict status checks go stale whenever the base branch moves, and with multiple agents merging PRs in parallel, every merge into main invalidates the "up to date" status of every other open PR — forcing a rebase and a full CI re-run on each sibling PR just to become mergeable again. That's real Action-minutes burned for no benefit, on top of being redundant with the required-status-check gate. GitHub also rejects self-APPROVE via the API, so `review.md` Step 10 force-downgrades an agent's review of its own PR to `COMMENT` with a `Verdict: APPROVE` marker — a required-review count can never be satisfied by the pipeline itself. Don't recommend merge-gating settings the pipeline structurally can't satisfy, that `--admin` bypasses anyway, or that actively waste CI budget.
...
- **Recommending required-review / enforce-admins / up-to-date as a blanket default regardless of who merges.** For a `shipwright-deploy`-enabled repo, `--admin` merges are the deploy model itself, not an emergency exemption (see Branch protection above), and it bypasses all three of these settings wholesale. Requiring ≥1 approval there doesn't add a review — self-APPROVE is blocked by GitHub's API — and "up to date" specifically burns real CI minutes on every sibling PR each time main advances, with nothing to show for it. Stricter-looking, not actually stricter. Match the recommendation to the repo's actual merge path.
- **Treating "require conversation resolution" the same as the settings above.** It's not bypass-relevant in the way that matters here — it costs zero CI minutes to enable and never blocks `deploy.md`'s `--admin` merge either way, so there's no stage-dependent reason to turn it off. Recommend it unconditionally rather than lumping it in with the settings that actually have a cost or a satisfiability problem.
