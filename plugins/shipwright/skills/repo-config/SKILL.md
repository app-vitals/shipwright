...
- **Require conversation resolution before merging** — **yes.** Zero-cost: it's GitHub review-thread metadata, not a CI job, so it burns no Action minutes to enable, and it doesn't block `deploy.md`'s `--admin` merge path either. A free safety net for any manual merge attempt.
...
- **Recommending required-review ≥1 / enforce-admins:true / require-up-to-date for a shipwright-deploy repo.** `--admin` merges (see `deploy.md` Step 4b) are the deploy model itself, not an emergency exemption, and it bypasses all three of these settings wholesale. Requiring ≥1 approval doesn't add a review — self-APPROVE is blocked by GitHub's API — and "up to date" specifically burns real CI minutes on every sibling PR each time main advances, with nothing to show for it. Stricter-looking, not actually stricter.
- **Treating "require conversation resolution" the same as the settings above.** It's not bypass-relevant in the way that matters here — it costs zero CI minutes to enable and never blocks `deploy.md`'s `--admin` merge either way. Recommend it unconditionally rather than lumping it in with the settings that actually have a cost or a satisfiability problem.
