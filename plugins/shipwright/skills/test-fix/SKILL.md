...
derivation as Step 3. Derive `repo-slug` the same way.
...
Use the **same field mapping as Step 5** above, sourced from the issue instead of a plan row:
...
- **Bulk append fails** (`/tasks/bulk` non-2xx, Step 7.1 or B5): log the response body and
  stop. Do not retry blindly; re-running the regular flow is idempotent because the dedup
  check (Step 4) skips already-queued rows. For backfill, a partial run is safe to re-run —
  already-closed issues are no longer in the open set fetched by B2, so they won't be
  reprocessed.
- **`gh` not authenticated or repo unreachable** (backfill only): log the failure and stop
  before creating or closing anything.
- **Issue missing its `<!-- task-id: T-NNN -->` marker** (backfill only): skip that issue,
  note it in the summary (B7), and do not close it — closing an unmigrated issue would lose
  its work with no task-store equivalent created.
