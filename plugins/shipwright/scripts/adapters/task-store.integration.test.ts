/**
 * plugins/shipwright/scripts/adapters/task-store.integration.test.ts
 *
 * Integration tests for TaskStoreHttpAdapter — cassette-backed fixtures in
...
import { TaskStoreHttpAdapter } from "./task-store.ts";
...
describe("TaskStoreHttpAdapter integration — query", () => {
  test("query({ ready: true }) makes GET /tasks?ready=true and returns tasks", async () => {
    ...
    const store = new TaskStoreHttpAdapter(CONFIG, fetchFn, TOKEN);
  ...
  test("query({ ready: true, assignee: 'dev' }) combines filters as ?ready=true&assignee=dev", async () => {
    const capturedPaths: string[] = [];
    const fetchFn: FetchFn = async (input, init) => {
      const { method, path } = resolveRequest(input, init);
      capturedPaths.push(`${method} ${path}`);
...
describe("TaskStoreHttpAdapter integration — append", () => {
  ...
  test("append with 409 conflict skips without error and returns inserted:0", async () => {
    ...
    // 409 = already exists — skip, do not count as inserted, do not throw
...
describe("TaskStoreHttpAdapter integration — update", () => {
  test("update(id, fields) sends PATCH /tasks/{id} and returns updated task", async () => {
    ...
    // Verify the PATCH body contains the fields
...
describe("TaskStoreHttpAdapter integration — claim", () => {
  test("claim(id) sends POST /tasks/{id}/claim and returns claimed task", async () => {
    ...
    const store = new TaskStoreHttpAdapter(CONFIG, fetchFn, TOKEN);
    // claim is called internally when we need to atomically acquire a task
    // we expose it via a dedicated method
...
describe("TaskStoreHttpAdapter integration — resolveRepo", () => {
...
describe("TaskStoreHttpAdapter integration — cleanup", () => {
  test("cleanup returns {closed:0, milestonesClosed:0, plansClosed:0} without HTTP calls", async () => {
    const capturedPaths: string[] = [];
    const fetchFn: FetchFn = async (input, init) => {
      const { method, path } = resolveRequest(input, init);
      capturedPaths.push(`${method} ${path}`);
...
describe("TaskStoreHttpAdapter integration — auth", () => {
  ...
  test("throws if token is missing", () => {
    expect(
      () => new TaskStoreHttpAdapter(CONFIG, makeCassetteFetch({}), ""),
