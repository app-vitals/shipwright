...
// CLI: bun run plugins/shipwright/scripts/classify_test_layer.ts <path>... or pipe paths via stdin.
...
// ─── Generic suffix-based defaults ────────────────────────────────────────────
// Project-agnostic layer definitions relying solely on file-suffix conventions.
// Directory-based prescriptions (e.g. time/**, scripts/**, lib/**) are intentionally
// omitted — they are project-specific and belong in docs/test-readiness/test-system.md.
//
// Precedence: smoke → e2e → unit → integration (most-specific suffix patterns first).
// The **/* variants are required because bare *.smoke.test.ts only matches root-level
// files (no / in path). The e2e/** directory convention is kept as it is generic.
...
//   Prefix match: "e2e/**" matches "e2e/anything"
...
function parseTestSystemMd(content: string): LayerDef[] {
  ...
  // Match table rows like: | **unit** | *.unit.test.ts |
...
// active defs. Using suffix patterns only (not directory patterns like "e2e/**")
