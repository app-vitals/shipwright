...
**Terminology note:** `dev-task`, `plan-session`, `review`, `patch`, and `deploy` are
implemented as commands (`commands/`), not skills (`skills/`). This document uses "skill" as a
conceptual label for any invocable workflow unit.
...
argument. `plan-session` (PDR-4.1) is the fifth loop-dispatched phase and carries the same
no-argument `[silent]` guard, but it is not *purely* explicit-target-only: invoked by a human
with a single `session` argument it auto-detects `repo` via `git remote get-url origin` and
waits for confirmation. Under `--autonomous` — the only mode `loop-orchestrator.ts` ever
dispatches, as `/shipwright:plan-session {repo} {session} --autonomous {task-id}` — both
`repo` and `session` are always passed explicitly by the dispatcher and that auto-detect path
must not run. None of the five self-scans or discovers its own work. Candidate selection
happens once, upstream, before any of these commands is even dispatched (see "Candidate Selection"
...
self-discovering inputs in any order, though — `dev-task`, `plan-session`, `review`, `patch`,
and `deploy` are dispatched with an explicit, already-selected target (a task id, a
repo/session pair, or a PR), chosen either by a human invoking the command directly or by the
`shipwright-loop` cron's `loop-orchestrator.ts`, which merges candidates from `agent/src`'s
per-phase qualification functions (`check-dev-task.ts`, `check-plan.ts`, `check-review.ts`,
`check-patch.ts`, `check-deploy.ts`) and picks exactly one winning item via
`work-selector.ts`'s `selectNextWorkItem` (strict age-based FIFO). What "no dependency between
skills" means in practice: none of the five
...
Selection of *what* to work on no longer happens inside `dev-task`, `plan-session`, `review`,
`patch`, or `deploy` themselves. It happens once, upstream, in `agent/src`'s native TypeScript
candidate providers — `check-dev-task.ts` (`getDevTaskCandidates`), `check-plan.ts`
(`getPlanCandidates`), `check-review.ts` (`getReviewCandidates`), `check-patch.ts`
(`getPatchCandidates`), and `check-deploy.ts` (`getDeployCandidates`). The `shipwright-loop`
cron's `loop-orchestrator.ts` calls the providers for every enabled phase each tick, merges
their results, and picks exactly one winning item via `work-selector.ts`'s
`selectNextWorkItem` (strict age-based FIFO across task and PR candidates, no phase-priority
bias). It then dispatches the one matching one-shot command with the winning item's target
embedded directly in the prompt — e.g. `/shipwright:dev-task {task-id}`,
`/shipwright:review {org/repo#number}`, or, for the plan phase,
...
`?kind=prd&status=pending` (TKD-1.3 dropped the legacy `autonomousPlanSession` boolean this query
used to send during TKD-1.1's transition window; `kind: "prd"` is now the only spelling), and
`check-dev-task.ts`'s `?ready=true` already excludes the `kind: "prd"` slice server-side
(task-store's `ready.ts`), so the two pools don't overlap in practice. It's kept because that
guarantee lives one deploy away — an agent binary running against a task-store version skew, or a
task whose `kind` was written inconsistently, can still surface the same task from both providers,
and the two copies' identical `createdAt` would otherwise let the untagged dev-task copy win the
FIFO tie.
...
`SHIPWRIGHT_AGENT_AUTONOMOUS_PLAN_SESSION_ENABLED` env var are on. With the env var unset the
plan branch is structurally unreachable and a tick behaves exactly as it did pre-PDR-4.1.
...
can also invoke any of the five commands directly with an explicit target (a task id, a
repo/session pair, or a PR) — the Independence Principles above still apply to that path. The
only thing that no longer exists is a mode where the command scans for its own candidates.

---

## System Cron Changes

System crons are the crons declared in an agent type manifest's `crons` array (`agent-types/{typeName}/manifest.yaml` — the `coding` type is the default) — they cover both the core shipwright loop (dev-task, plan, review, patch, deploy) and maintenance tasks (entropy patrol, docs freshness, etc.). The manifest is the source of truth: `AgentCronJobService.reconcileSystemCrons()` resolves an agent's `typeName` to its manifest via the `AgentTypeRegistry` and seeds/updates each agent's system crons from it. When a shipwright change adds, removes, or restructures any system cron, flag two questions during planning:
...
**New system crons always ship disabled.** Any cron added to a manifest's `crons` array must have `enabled: false`. Enable it per-agent when ready. This avoids new crons firing unexpectedly on agents that haven't opted in. The exception is crons explicitly replacing a prior cron — but even then, add disabled, verify, then enable and disable the old one.

**dev-task, plan, review, patch, and deploy are phases dispatched by `shipwright-loop`, not independent self-discovering crons.** `shipwright-dev-task`, `shipwright-plan`, `shipwright-review`, `shipwright-patch`, and `shipwright-deploy` are declared in the agent type manifest's `crons` array with `parentCron: "shipwright-loop"` (see `agent-types/coding/manifest.yaml`). `shipwright-loop`'s `loop-orchestrator.ts` is the sole dispatcher — see the Candidate Selection Contract above: it calls each phase's candidate provider every tick, merges the results, picks exactly one winning item via `selectNextWorkItem`, and runs the one matching command with the target embedded. `dev-task`, `review`, and `patch` ship `enabled: true` by default; `deploy` and `plan` ship `enabled: false` (explicit opt-in), and `plan` additionally requires the `SHIPWRIGHT_AGENT_AUTONOMOUS_PLAN_SESSION_ENABLED` env var. None of the five scans for its own candidates.
