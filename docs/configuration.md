...
| `auto_merge_any_author_repos` | `string[]` | `[]` | A list of "org/repo" strings (AM-1). Read by `agent/src/check-deploy.ts`'s `getDeployCandidates()` via `readAutoMergeAnyAuthorRepos()`/`isAutoMergeAnyAuthorRepo()`. When a repo is on this list, the hard authorship filter is skipped for PRs in that repo only — a PR authored by anyone (not just the agent) can become a deploy candidate. The existing approval gate (GitHub-native `reviewDecision: "APPROVED"`, or self-review per `allow_self_review`) still applies unconditionally. Empty/absent (the default) reproduces today's behavior exactly for every repo — only PRs authored by the agent's own GitHub identity are deploy candidates. |
| `auto_merge_hold_label` | `string` | `""` (unset) | A single GitHub label name (AM-1). Read by `agent/src/check-deploy.ts`'s `getDeployCandidates()` via `readAutoMergeHoldLabel()`/`isHeldByLabel()`. Any open PR carrying this label is excluded from deploy candidacy, regardless of approval/CI/authorship. Matched via exact string comparison against the PR's labels (case-sensitive — mirrors `agent/src/check-review.ts`'s pre-existing `automated-label` exclusion, the only label-comparison precedent in this codebase). Re-checked immediately before the actual merge (`/shipwright:deploy`'s Step 4a.5) to close the window where the label is added to the PR after candidate selection but before the merge executes. Empty/absent (the default) is a no-op — an empty string never matches a real label. |
