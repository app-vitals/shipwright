...
| `patchAuthorAllowlist` | `string[]` | `[]` | GitHub login strings permitted to trigger this agent's patch operations. **Empty means unfiltered** — every authenticated author is allowed. Settable at creation (admin UI create-form field) and editable via `PATCH /agents/:id` or the admin UI. |
