...
| `SHIPWRIGHT_TASK_STORE_TOKEN` | `string` | — | Bearer token for task-store API access. Minted per-agent by the admin provisioner and stored in the agent Secret (key `task-store-token`); injected into the agent Deployment via `SHIPWRIGHT_TASK_STORE_TOKEN` env var. Used by agents to claim tasks, update status, and query the task queue. Read directly by the agent's in-process candidate providers in `agent/src/check-helpers.ts` — `createTaskStoreClient()` (used by `check-dev-task.ts`), `createPrRecordQuery()` (used by `check-review.ts`, `check-patch.ts`, and `check-deploy.ts`), and `createTaskStatusQuery()` (used by `check-review.ts` and `check-patch.ts` to source candidate age from task.createdAt per LPF-3.2) — for the `shipwright-loop` cron. Env-var-only (secret). |
