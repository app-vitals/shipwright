...
| `SENTRY_DSN` | `string` | — | Sentry error reporting DSN for the task-store and metrics services. When set, these services initialize Sentry and mount error-capture middleware that reports 5xx and unhandled exceptions. When unset, Sentry is disabled (zero telemetry overhead). The same DSN is also used by the Hono middleware (`@sentry/hono`) to initialize Sentry with scrubbing hooks that redact secrets and environment-specific tags. Env-var-only (secret). |
| `SENTRY_ENVIRONMENT` | `string` | — | Sentry environment tag for the task-store and metrics services. Defaults to `NODE_ENV` if unset, then `production`. Passed as the `environment` field in Sentry init options. Optional alongside `SENTRY_DSN`. |
