...
| `SHIPWRIGHT_TASK_STORE_CLAIM_TTL_MS` | `number` | `900000` | Milliseconds an agent's task claim remains valid without a heartbeat. When an agent's last heartbeat exceeds this TTL, the claim will be abandoned and the task eligible for re-claiming by another agent. |
