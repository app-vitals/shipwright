...
| `reviewAuthorAllowlist` | `string[]` | `[]` | GitHub login strings permitted to trigger this agent's review/dev-task work. **Empty means unfiltered** — every authenticated author is allowed. Settable at creation (admin UI create-form field) and editable afterward (admin UI agent-detail page's add/delete rows, or `PATCH /agents/:id`). For local dev, `hitl.ts`'s `SHIPWRIGHT_HITL_AUTHORS` env var is the equivalent — it stays in sync with the persisted record via `PATCH` (see the `SHIPWRIGHT_HITL_AUTHORS` row under [Dev-only](#dev-only)). This was previously named `authorAllowlist`; that column was removed in DBR-2.4. |
| `patchAuthorAllowlist` | `string[]` | `[]` | GitHub login strings naming additional authors whose PRs this agent will treat as patch candidates. **DBR-1.3:** synced live via `agent/src/patch-author-allowlist-ref.ts`. **DBR-1.4:** enforcement is active — `check-patch.ts` adds PRs authored by allowlisted logins to the candidate pool (merged with self-authored, deduplicated by repo/number). Empty allowlist means self-authored-only (additive source, not fail-open). Settable at creation and editable via `PATCH /agents/:id`; not yet surfaced as an admin UI form field. |
...
| `SHIPWRIGHT_HITL_AUTHORS` | `string` | — | Comma-separated list of GitHub login strings; when set, restricts review candidates to PRs authored by one of these users (default: none, unfiltered). Equivalent to the agent's `reviewAuthorAllowlist` config field; `hitl.ts` syncs this value onto the persisted hitl agent record via `PATCH /agents/:id`. |
