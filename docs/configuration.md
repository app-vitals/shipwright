...
| `SENTRY_DSN` | `string` | — | Sentry error reporting DSN, read by both the task-store and metrics services. When set, the service initializes Sentry and mounts error-capture middleware (`@sentry/hono`) that reports 5xx/unhandled exceptions. When unset, Sentry is disabled (zero telemetry overhead) for that service. The same DSN also drives scrubbing hooks that redact secrets and environment-specific tags. Env-var-only (secret). |
| `SENTRY_ENVIRONMENT` | `string` | — | Sentry environment tag, read by both the task-store and metrics services. Defaults to `NODE_ENV` if unset, then `production`. Passed as the `environment` field in Sentry init options. Optional alongside `SENTRY_DSN`. |
