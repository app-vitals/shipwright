...
| `patchAuthorAllowlist` | `string[]` | `[]` | GitHub login strings intended to gate this agent's patch operations. **Stored only — not enforced yet.** DBR-1.1 adds the column and admin API field; no runtime code reads it (there is no patch equivalent of `review-author-allowlist-ref.ts`, and `check-patch.ts` does no allowlist filtering), so patch runs are unfiltered whatever this holds. Settable at creation and editable via `PATCH /agents/:id`; not yet surfaced as an admin UI form field. |
