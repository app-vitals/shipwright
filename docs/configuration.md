...
| `authorAllowlist` | `string[]` | `[]` | GitHub login strings permitted to trigger this agent's review/dev-task work. **Empty means unfiltered** — every authenticated author is allowed. Settable at creation (admin UI create-form field) and editable afterward (admin UI agent-detail page's add/delete rows, or `PATCH /agents/:id`). For local dev, `hitl.ts`'s `SHIPWRIGHT_HITL_AUTHORS` env var is the equivalent — it stays in sync with the persisted record via `PATCH` (see the `SHIPWRIGHT_HITL_AUTHORS` row under [Dev-only](#dev-only)). **DBR-2.1 transitional note:** `reviewAuthorAllowlist` is the rename-in-progress twin; both fields always hold the same value and are dual-written. Prefer using `reviewAuthorAllowlist` in new code. |
| `reviewAuthorAllowlist` | `string[]` | `[]` | GitHub login strings permitted to trigger this agent's review/dev-task work — the canonical name for the allowlist field (DBR-2.1 rename in progress). Identical to `authorAllowlist` during the migration. Preferred over `authorAllowlist` in new code; `authorAllowlist` is kept in sync and will be removed after the migration completes. |
| `patchAuthorAllowlist` | `string[]` | `[]` | GitHub login strings intended to gate this agent's patch operations. **DBR-1.3:** synced live via `agent/src/patch-author-allowlist-ref.ts`. **DBR-1.4:** enforcement is active — `check-patch.ts` filters patch candidates to PRs authored by allowlisted logins (merged with self-authored, deduplicated by repo/number). Empty allowlist means self-authored-only (additive source, not fail-open). Settable at creation and editable via `PATCH /agents/:id`; not yet surfaced as an admin UI form field. |
...
| `SHIPWRIGHT_RECONCILER_UPDATED_SINCE_WINDOW_MS` | `number` | `21600000` (6h) | Recency window (PSR-1.1) for the PR state reconciler's `updatedSince` filter, applied to `reconcilePrState`'s open-record scan and `reconcileReviewState`'s pending-record scan — each pass only re-fetches records updated within this window instead of unconditionally rescanning every record every tick, avoiding the same GraphQL rate-limit exhaustion `SHIPWRIGHT_RECONCILER_DELAY_MS` mitigates. Deliberately wide relative to `SHIPWRIGHT_PR_STATE_RECONCILER_INTERVAL_MS`'s 30-60 minute tick interval: these passes exist specifically to heal a task-store record whose `updatedAt` will never change again once truly stuck by a crash/hang, so the window only needs to survive a missed or delayed tick (restart, deploy) rather than exclude every record that isn't brand-new. `listPostedReviewRecords` (CHU-2.4) is permanently exempt from this filter regardless of window size, and as of RCB-1.1 so is `reconcilePrOpenTasks` (see `SHIPWRIGHT_RECONCILER_PR_OPEN_TASK_BATCH_SIZE` below) — see the module doc comment in `agent/src/pr-state-reconciler.ts`. Env-var-only; read once at process start. |
| `SHIPWRIGHT_RECONCILER_PR_OPEN_TASK_BATCH_SIZE` | `number` | `200` | Per-tick fetch cap (RCB-1.1) for `reconcilePrOpenTasks`'s batch + rotating-offset cursor scan of task-store Tasks with `status:"pr_open"`. Replaces an earlier `updatedSince` recency filter for this one pass specifically: nothing ever bumps a pr_open task's `updatedAt` while it sits waiting on human review, so a PR that took longer than the recency window to merge would permanently fall out of every future scan and never get reconciled — even after merging on GitHub (confirmed live: over a dozen tasks stuck this way after a slow review turnaround). Each call fetches up to this many tasks (paginated in the usual 50-record page) starting at an in-process, module-level rotating offset cursor; when a call's fetch comes up short of the batch size, the cursor wraps back to 0 so the next call restarts from the beginning — a round-robin sweep across ticks that eventually covers the whole list without ever permanently excluding a stuck record. The cursor is process-local (not persisted) and resets to 0 on restart. Env-var-only; read once at process start. |
| `SHIPWRIGHT_RECONCILER_SCOPE_DEGRADED_RETRY_DELAY_MS` | `number` | `1000` (1s) | Bounded pause before `listAllTasksForBranch`'s single retry (RSG-1.2) of a task-store `GET /tasks?repo=&branch=` response that reported `scopeDegraded:true` (RSG-1.1 — set when the calling agent token's own repo-scope resolver call failed, which forces `repos` to `[]` server-side and can silently under-count or zero out real bundle-mate tasks sharing a branch). The BBR-1.1 bundle-mate guard call sites inside `reconcilePrOpenTask`'s direct and branch-fallback paths depend on this count being trustworthy before auto-healing a branch-only PR match; if the retried response is STILL degraded, the true sibling count is treated as unknown and the auto-heal is skipped (same effective outcome as a confirmed `>1` sibling count), logging a message distinct from the existing "N tasks share this branch" skip so the two causes stay distinguishable in practice. A non-degraded response (including a legitimate 0- or 1-task result) never triggers this delay at all. Production uses a real `setTimeout`-backed pause (reusing the same injected `delay` dep PSR-1.2 already threads through this file); tests inject a no-op. Env-var-only; read once at process start. |
...
| `SHIPWRIGHT_CHAT_PUSH_WEBHOOK_URL` | `string` | — | Full endpoint URL of the admin service's inbound push webhook (e.g. `https://admin.example.com/admin/push/notify`) — **not** a base URL. When set alongside `SHIPWRIGHT_CHAT_PUSH_WEBHOOK_TOKEN`, the chat service fires a fire-and-forget, text-free `{ threadId, agentId, title }` POST to this URL after each reply persists, to trigger a "your agent replied" push notification. A dedicated outbound notifier — mirrors the scope resolver's env-gated pattern but with its own credential (see the token row below). A push failure never fails the reply: errors and non-2xx responses are logged and swallowed. Optional — when unset, the notifier is disabled and logged as such. |
| `SHIPWRIGHT_CHAT_PUSH_WEBHOOK_TOKEN` | `string` | — | Bearer token the chat service presents to the admin `POST /admin/push/notify` webhook. Required alongside `SHIPWRIGHT_CHAT_PUSH_WEBHOOK_URL` to enable the outbound reply notifier. Env-var-only (secret). A distinct credential from `SHIPWRIGHT_CHAT_AGENTS_API_KEY` (scoped to reading agent metadata) — in deployment it is set to the same shared-secret value as the admin side's `SHIPWRIGHT_ADMIN_PUSH_WEBHOOK_TOKEN`. |
| `CHAT_STALLED_AFTER_MS` | `number` | `300000` (5 min) | Stall threshold in milliseconds for the chat service's background stall-reaper job (runs every 60s). A claimed, unreplied user message is terminalized with `errorKind: "stalled"` when its `heartbeatAt` (or `claimedAt` if the agent died before its first beat) is older than this cutoff. Deliberately 2.5x the admin UI's 120s stall-warning threshold — a warning is free, but reaping is destructive (marks the message stalled; never unclaims it for retry). The reaper reuses `MessageService.reply()`, which is idempotent (returns `null` if already replied), making the sweep race-safe against an agent that resumes and safe across multiple replicas. See `chat/src/stall-reaper.ts` and `chat/src/main.ts` for implementation details. Optional — when unset, defaults to `300000` (5 min). |
...
| `SHIPWRIGHT_ADMIN_APP_BASE_URL` | `string` | (auto-derived or `http://localhost:{PORT}`) | Public base URL of the admin service, used to construct the OAuth/OIDC redirect URIs for both Google and Okta. An explicit value always wins. When empty and deployed via Helm with `networking.type=ingress` or `networking.type=gateway`, auto-derives from the ingress/gateway host (with HTTPS when TLS is on). For ClusterIP/NodePort/LoadBalancer installs, the env var stays omitted unless set explicitly. In bare-metal deployment, defaults to `http://localhost:{PORT}`. |
...
| `SHIPWRIGHT_ADMIN_VAPID_PUBLIC_KEY` | `string` | — | VAPID public key (base64url, uncompressed P-256 point) for Web Push notifications on agent replies. Sent to the browser (NOT a secret). Web Push is enabled only when public + private + subject are ALL set; otherwise the push routes 503, the chat page renders no toggle, and the app degrades to the poll-only experience. |
| `SHIPWRIGHT_ADMIN_VAPID_PRIVATE_KEY` | `string` | — | VAPID private key (base64url raw P-256 scalar) used to sign the RFC 8292 VAPID JWT. Env-var-only (secret; masked in admin env responses and scrubbed by Sentry). Required alongside the public key + subject to enable Web Push. |
| `SHIPWRIGHT_ADMIN_VAPID_SUBJECT` | `string` | — | VAPID `sub` claim — a `mailto:` address or an https URL identifying the push sender per RFC 8292. Required alongside the public + private key to enable Web Push. |
| `SHIPWRIGHT_ADMIN_PUSH_MAX_DETAIL` | `generic` \| `title` \| `preview` | `title` | Operator hard ceiling for how much detail a push notification may reveal on a locked screen. `generic` = "Your agent replied" only; `title` = + thread title; `preview` = + up to 120 chars. The effective per-notification level is `min(this ceiling, the subscription's own opt-in)`. Agent names, ids, repo names, file paths and costs are NEVER included at any level. |
| `SHIPWRIGHT_ADMIN_PUSH_WEBHOOK_TOKEN` | `string` | — | Shared bearer token the chat service presents to the admin `POST /admin/push/notify` webhook to trigger a "your agent replied" notification. Env-var-only (secret). Required (alongside a full VAPID config) for the inbound reply trigger. |
