...
| `patchAuthorAllowlist` | `string[]` | `[]` | GitHub login strings permitted to trigger this agent's patch operations. Synced live via `agent/src/patch-author-allowlist-ref.ts` (DBR-1.3). Settable at creation and editable via `PATCH /agents/:id`; not yet surfaced as an admin UI form field. |
