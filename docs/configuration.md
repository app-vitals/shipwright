...
| `SHIPWRIGHT_TASK_STORE_CLAIM_TTL_MS` | `number` | `2100000` | Milliseconds a claim (Task or PR review/patch/deploy) remains valid without a heartbeat. When an agent's last heartbeat exceeds this TTL, the claim will be abandoned and the record eligible for re-claiming by another agent. A single unified TTL covers both Task claims (`/shipwright:dev-task`) and PR claims — 2 100 000 ms (35 min) is the 30-minute `claude -p` hard timeout plus a 5-minute buffer. |
