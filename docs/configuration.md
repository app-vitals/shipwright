...
| `patchAuthorAllowlist` | `string[]` | `[]` | GitHub login strings intended to gate this agent's patch operations. **Not enforced yet.** DBR-1.3 syncs the value live via `agent/src/patch-author-allowlist-ref.ts`, but `check-patch.ts` still does no allowlist filtering, so patch runs are unfiltered whatever this holds. Settable at creation and editable via `PATCH /agents/:id`; not yet surfaced as an admin UI form field. |
