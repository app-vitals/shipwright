...
`GET`/`POST`/`PATCH`/`DELETE /agents[/:id]`, `POST /agents/:id/provision`, and `POST /agents/reconcile` are fully described in the OpenAPI spec — including the rollback-guarded provisioning block. Field semantics the spec doesn't carry:

`POST /agents` is **admin-only** (`isAdmin=true` — admin key or session cookie; per-agent bearer tokens cannot call it) and shares its implementation with the web UI form at `/admin/agents/new`: both call the same `createAgent()` function (`admin/src/agents.ts`), which validates name/typeName, creates the Agent row, seeds AgentTool/AgentPlugin rows from the selected Agent Type manifest (member emails and repos come from their own dedicated fields, not the manifest — no Agent Type shipped today populates a `members`/`repos` array), and (for non-self-hosted agents) provisions the Kubernetes workload. The request body takes the same raw, unparsed fields the form submits (e.g. newline-separated `reposRaw`/`authorAllowlistRaw`); creation is transactional — any failure (missing fields, unresolvable `typeName`, malformed repo/allowlist lines, provisioning failure) leaves zero rows behind rather than a partial-success 200. See the [`agent-admin`](../plugins/shipwright/skills/agent-admin/SKILL.md) skill for the walkthrough.
