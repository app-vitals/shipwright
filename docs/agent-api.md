...
`patchAuthorAllowlist` is an array of GitHub login strings — the authors permitted to trigger patch operations against this agent. Synced live via `agent/src/patch-author-allowlist-ref.ts` and enforced in `check-patch.ts` (DBR-1.3). When empty, all authenticated users are allowed.
...
- `patchAuthorAllowlist` — array of GitHub login strings (the authors permitted to trigger patch operations on this agent). Synced and enforced by the runtime via `agent/src/patch-author-allowlist-ref.ts` (DBR-1.3). When empty, all authenticated users are allowed.
