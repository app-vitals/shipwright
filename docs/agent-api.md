...
- `patchAuthorAllowlist` — array of GitHub login strings (the authors intended to be permitted to trigger patch operations on this agent). **DBR-1.3:** synced live via `agent/src/patch-author-allowlist-ref.ts`. **DBR-1.4:** enforcement is active — patch candidates are filtered to allowlisted authors (merged with self-authored, deduplicated by repo/number). Empty allowlist means self-authored-only (additive source, not fail-open).
