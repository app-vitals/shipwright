...
| `admin/src/admin-ui.ts` | Admin UI factory `createAdminUIApp()` — server-rendered Hono app (login, agent list/detail, Slack provisioning, tasks) with POST mutation routes for cron jobs, tools, tokens (create/toggle/delete/revoke), and tasks (release). Accepts `devAuthEnabled`, `fetchTaskStoreTasks`, `releaseTask` in `AdminUIDeps`; when `devAuthEnabled=true`, registers `GET /admin/dev-login` (dev auto-login). When `fetchTaskStoreTasks` is provided, `GET /admin/tasks` queries tasks; without it, the page renders in degraded mode. |
...
| `admin/src/admin-ui-pages.ts` | Page rendering functions (`renderLoginPage`, `renderAgentsPage`, `renderAgentDetailPage`, `renderProvision*`, `renderTasksPage`). Exports `TaskItem` interface for typing tasks displayed on the admin UI. |
