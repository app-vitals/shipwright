...
| `admin/src/admin-ui-pages.ts` | Page rendering functions (`renderLoginPage`, `renderAgentsPage`, `renderAgentDetailPage`, `renderProvision*`, `renderTasksPage`, `renderPrsPage`, `renderPrDetailPage`). Exports `TaskItem` interface for typing tasks displayed on the admin UI, and `PrListItem` interface for PR data displayed on PR list/detail pages. |
