...
| GET | `/admin/chat/:agentId/threads/:threadId` | admin | View a single thread with its messages. Renders thread title, message history with message bubbles (role-labeled, color-coded by role, markdown-rendered for assistant messages), rename form, delete button, and a send form. Client-side JavaScript handles message sending and polling for real-time updates (`/admin/chat/:agentId/threads/:threadId/messages.json`). A sidebar pane lists all threads for the agent. Returns `text/html`. |
...
| POST | `/admin/chat/:agentId/threads/:threadId/messages.json` | admin | JSON API: add a message to a thread. Body: `{ body: string }`. Returns `{ message: ChatMessage }` (200) or `{ message: null }` (200/400/500 on chat service error, client determines retry). Enables client-side send + optimistic UI + polling loop. |
