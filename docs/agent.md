...
| Chat Tokens | `POST /agents/:id/chat-tokens/daily` (fire-and-forget daily upsert: accumulates Slack chat session token usage by date and agent; body: `{date: YYYY-MM-DD, inputTokens, outputTokens, cacheReadTokens, cacheCreationTokens, costUsd, model}`; returns `{row: {id, ...}}` on success or error response on failure) |
...
| `agent/src/chat-token-reporter.ts` | Chat token reporter: `ChatTokenReporter` interface (async `recordSession(usage?)` method) + `HttpChatTokenReporter` (POSTs per-session Slack chat token usage to `POST /agents/:id/chat-tokens/daily`) and `NoopChatTokenReporter` (test/default no-op). Injected into Slack event handler; called at each Slack session completion (DM, mention, reaction) to report accumulated input/output/cache token counts, cost, and model. Helper `formatDailyDate(date, timeZone?)` formats dates as YYYY-MM-DD in a given IANA timezone (agent timezone when unspecified). Fire-and-forget design: swallows POST errors as warnings to `console`. |
| `agent/src/slack.ts` | Slack event handler: `createSlackApp()` — Bolt-based Socket Mode app handling DMs, `app_mention`, `reaction_added`, file attachments, and voice transcription. Accepts injected `ChatTokenReporter` (default `NoopChatTokenReporter`) and calls it at session completion. |
