...
| **C** | **Shipwright agent** | `agent/` + `admin/` | Hono service + Prisma store (in `@shipwright/admin`); a thin autonomous runner: pick next ready task → build → ship PR → forward metrics. Four HTTP surfaces: runtime API, admin CRUD API, admin UI, and public read-only task board. |
