...
**Crucially**, `Bash`, `WebSearch`, `WebFetch`, `Agent`, and `Monitor` are **not** in the floor set. These are high-privilege tools (arbitrary system execution, external HTTP, sub-agent spawning, and streaming monitoring) and must be explicitly seeded into the agent's `AgentTool` table to be available. An agent with no `AgentTool` rows (or a 404'd config bundle from the admin API) receives only the 7 floor tools and cannot execute shell commands, fetch external URLs, dispatch sub-agents, or stream monitoring data.
...
|---|---|---|---|
| `shipwright-plan` | `* * * * *` (every minute) | off | Runs an autonomous planning session for the PRD task `shipwright-loop` selects (dispatched as `/shipwright:plan-session {repo} {session} --autonomous {task-id}`). Candidates come from `agent/src/check-plan.ts` — task-store tasks recorded as `kind: "prd"` and still `pending`, carrying both `repo` and `session` (a PRD task missing either is dropped). The spec is read from `planning/{session}/PRODUCT-SPEC.md` in the worktree, or materialized there from the task's `description` when the file is absent (plan-session Step 1 `--autonomous` Mode). **Double-gated:** this cron toggle AND the `SHIPWRIGHT_AGENT_AUTONOMOUS_PLAN_SESSION_ENABLED` env var must both be on; with the env var unset the loop never collects or dispatches a plan candidate. Standalone (loop disabled): inert — see above. |
...
POM-4.1's repo-wide merged-PR origin census (`agent/src/pr-census.ts`, `runPrCensus`) is not a standalone cron — it's a fifth independent pass on the existing PR-state-reconciler's tick (see `SHIPWRIGHT_PR_STATE_RECONCILER_INTERVAL_MS` in [configuration-agent.md](./configuration-agent.md#metrics--admin--chat--task-store-services)), reinstating visibility into merged PRs the pipeline never claims directly — e.g. a human merging their own PR without ever invoking `/shipwright:review`, or a dependency-bot PR merged untouched.

Each tick, per scoped repo, the sweep reads the repo's incremental search-window cursor (`GET /prs/census/cursor`, POM-1.1 — the max `mergedAt` among rows with a non-null `origin`). **A `null` cursor (the repo's first run) issues zero `gh`/task-store calls for that repo**, rather than backfilling full PR history — full backfill for historical PRs before origin-tracking was deployed is handled separately by POB-1.1's `agent/scripts/backfill-pr-origin.ts` CLI script. The cursor is derived server-side, not written explicitly, so the very next tick after the first shipwright-origin PR lands for that repo naturally sees a real cursor.

Otherwise, the sweep issues exactly **one `gh pr list --state merged --search "merged:>=<cursor>"` call per scoped repo per tick** — GitHub's Search API (30 req/min, distinct from the general 5000/hr REST limit) — classifies each result via the label-free `classifyPrOrigin()` precedence (see [metrics.md](./metrics.md#origin-classification-rules)), and `POST`s the classified batch to `/prs/census` in chunks of at most 200 entries. A per-repo failure is caught, logged, and does not stop the other scoped repos in the same tick.

Configured via `SHIPWRIGHT_AGENT_PR_CENSUS_ENABLED` (kill switch) and `SHIPWRIGHT_AGENT_PR_CENSUS_INTERVAL_MS` (optional additional throttle beyond the shared reconciler tick cadence) — see [configuration-agent.md](./configuration-agent.md#metrics--admin--chat--task-store-services) for the full reference; not duplicated here.
