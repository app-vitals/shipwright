...
|---|---|---|---|
| `shipwright-plan` | `* * * * *` (every minute) | off | Runs an autonomous planning session for the PRD task `shipwright-loop` selects (dispatched as `/shipwright:plan-session {repo} {session} --autonomous {task-id}`). Candidates come from `agent/src/check-plan.ts` — task-store tasks recorded as `kind: "prd"` (the legacy `autonomousPlanSession: true` flag is normalized to it on write) and still `pending`, carrying both `repo` and `session` (a PRD task missing either is dropped). The spec is read from `planning/{session}/PRODUCT-SPEC.md` in the worktree, or materialized there from the task's `description` when the file is absent (plan-session Step 1 `--autonomous` Mode). **Double-gated:** this cron toggle AND the `SHIPWRIGHT_AGENT_AUTONOMOUS_PLAN_SESSION_ENABLED` env var must both be on; with the env var unset the loop never collects or dispatches a plan candidate. Standalone (loop disabled): inert — see above. |
...
| `shipwright-site-docs-freshness` | `0 7 * * *` (daily, 07:00) | off | Scans the marketing site's pages in `site/src/content/docs/` for source changes and refreshes pages that drifted from their source docs via the site-docs-sync command (`docs-sync --auto`). Uses page-scoped anchors in `state/site-docs-last-synced.json`, checking each page's mapped sources (from `site/docs-source-map.json`) against its own last-synced SHA. Pages are evaluated independently; a pre-check failure for one page does not block others. See [`docs/site-docs-freshness.md`](./site-docs-freshness.md) for the source-map format and precheck contract. |
