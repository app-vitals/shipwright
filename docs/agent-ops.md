...
Each tick, per scoped repo, the sweep reads the repo's incremental search-window cursor (`GET /prs/census/cursor`, POM-1.1 — the max `mergedAt` among rows with a non-null `origin`). **A `null` cursor (the repo's first run) issues zero `gh`/task-store calls for that repo**, rather than backfilling full PR history — full backfill is a separate, explicitly out-of-scope concern. The cursor is derived server-side, not written explicitly, so the very next tick after the first shipwright-origin PR lands for that repo naturally sees a real cursor.
