...
Body (JSON): task fields. `title`, `status`, and `repo` are required. Agent tokens force `assignee` to their own ID. Returns `201` with the created task.
...
Body: JSON array of task objects. Each task must have `title`, `status`, and `repo` fields. Skips conflicts (existing ID) rather than failing. Returns `{ inserted: number, updated: number }`.
