...
1. **Absolute threshold** (`scripts/check-coverage.ts`): enforced by the main `ci` job on the LCOV report produced by `task test:coverage`, requiring 80% lines + 80% functions on an aggregate basis.
