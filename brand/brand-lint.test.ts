...
test("an artifact using only token hexes passes clean", () => {
  const onBrand = "a { color: #34C77B; } body { background: #080E1E; } .x { color: #FFFFFF; }";
  expect(lintBrand(onBrand).offBrandHexes).toEqual([]);
});

test("artifacts using CSS variables (no raw hexes) pass clean", () => {
  const viaVars = ".sw-btn { background: var(--sw-color-brand-default); color: var(--sw-color-brand-on-brand-text); }";
...
test("arbitrary off-palette colors are flagged", () => {
  const { offBrandHexes } = lintBrand("h1 { color: #FF0000; } p { color: #123ABC; }");
