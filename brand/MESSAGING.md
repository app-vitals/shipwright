...
- **Designated surfaces only:** competitor names may appear on `/vs/*` pages (e.g. `/vs/devin`), `/self-hosted`, `/compare` (the deliberate exception page for its full landscape table and market-poles framing), the homepage H1/meta phrase itself (plus one linked, factual sentence), and the global site nav and footer (`BaseLayout.astro`) — limited to a single "vs Devin" link in each, pointing to `/vs/devin`, with no additional claim text. No other surface may name a competitor — this includes the rest of the homepage body (the `#differentiators` section stays competitor-free), blog posts, and README.
