...
- Gateway API networking: setting `networking.type=gateway` renders a `gateway.networking.k8s.io/v1` `Gateway` and `HTTPRoute`(s) instead of an Ingress. Mutually exclusive with `networking.type=ingress`. Requires Gateway API CRDs in the cluster.
- cert-manager Certificate: setting `tls.certManager.enabled=true` renders a `cert-manager.io/v1` `Certificate` for `networking.gateway.host`, wired to a configurable `issuerRef`. When enabled the Gateway also adds an HTTPS (`:443`) listener. Disabled by default.
- Agent-provisioning RBAC: `agent.provisioning.enabled` (default `false`) renders a namespace-scoped `Role`, `RoleBinding`, and agent `ServiceAccount`, plus the provisioner env contract (`SHIPWRIGHT_K8S_PROVISIONING`, `SHIPWRIGHT_K8S_NAMESPACE`, `SHIPWRIGHT_AGENT_IMAGE`, etc.) in the admin Deployment. Nothing is rendered and the admin service stays in Noop mode when disabled.
- `metrics.provider` block: configurable PostHog provider (`posthog.existingSecret`, `posthog.personalApiKeyRef`, `posthog.projectIdRef`) for `POSTHOG_PERSONAL_API_KEY` / `POSTHOG_PROJECT_ID` injection via `secretKeyRef`, plus `adminUrl`, `basePath`, `requireOwnerRole`, and `internalKey.*` environment controls.
