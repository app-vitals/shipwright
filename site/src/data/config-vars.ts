...
export const agentTaskStoreVars: ConfigVar[] = [
  ...
  { name: "SHIPWRIGHT_TASK_STORE_ADMIN_TOKEN", type: "string", def: "—", desc: "Bearer token for admin-side task-store token minting. Required (alongside SHIPWRIGHT_TASK_STORE_URL) to enable per-agent provisioning at agent-creation time. Env-var-only (secret)." },
