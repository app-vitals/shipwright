...
export const agentProvisioningVars: ConfigVar[] = [
  ...
  { name: "SHIPWRIGHT_AGENT_DEFAULT_ANTHROPIC_API_KEY", type: "string", def: "", desc: "Deployment-wide Anthropic API key handed to every agent via its config bundle, merged underneath that agent's own env rows (a per-agent ANTHROPIC_API_KEY always wins). Read by the admin service. Deliberately prefixed: admin must never hand out a bare ANTHROPIC_API_KEY it holds for its own use. Blank is treated as unset. Set via the chart's agent.credentials.anthropicApiKey / .existingSecret." },
  { name: "SHIPWRIGHT_AGENT_DEFAULT_CLAUDE_CODE_OAUTH_TOKEN", type: "string", def: "", desc: "As above, for a Claude Code OAuth token (claude /oauth-token). Becomes CLAUDE_CODE_OAUTH_TOKEN in each agent's config bundle. Set via the chart's agent.credentials.claudeCodeOauthToken." },
