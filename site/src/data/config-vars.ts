...
export const agentTaskStoreVars: ConfigVar[] = [
  ...
  { name: "SHIPWRIGHT_TASK_STORE_CLAIM_TTL_MS", type: "number", def: "900000", desc: "Milliseconds a PR review/patch/deploy claim remains valid without a heartbeat. When an agent's last heartbeat exceeds this TTL, the claim will be abandoned and the PR record eligible for re-claiming by another agent. Task claims use SHIPWRIGHT_TASK_STORE_TASK_CLAIM_TTL_MS instead." },
  { name: "SHIPWRIGHT_TASK_STORE_TASK_CLAIM_TTL_MS", type: "number", def: "1800000", desc: "Milliseconds an agent's task claim (/shipwright:dev-task) remains valid without a heartbeat. When an agent's last heartbeat exceeds this TTL, the claim will be abandoned and the task eligible for re-claiming by another agent. Set higher than SHIPWRIGHT_TASK_STORE_CLAIM_TTL_MS because dev-task implementation work has wider duration variance than review." },
