...
A task typically has one associated PR record once it reaches `pr_open`. The link is optional — either resource can exist without the other. There is no `taskId` field on the PR record — the association is resolved live by matching `(Task.repo, Task.pr)`, the same pairing `GET /tasks?repo=&pr=` exposes.
...
**Updatable fields:** `staged`, `commitSha`, `agentId`, `state`, `mergedAt`, `reviewState`
