...
4. **Check the work queue.** The **Upcoming** section of the same
   ...
   the tasks and PRs it sees as pending work, oldest first, across the dev-task, review, patch, and
   deploy phases. An empty queue with no snapshot yet usually means the agent hasn't ticked since
   this feature shipped, or `shipwright-loop` isn't enabled.
...
own ranked view of what it intends to work on next, oldest first, across all four pipeline phases —
