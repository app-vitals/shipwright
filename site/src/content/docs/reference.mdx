...
<ConfigTable vars={agentServicesVars} />

### Agent provisioning (Kubernetes)

Controls how the admin service provisions the Kubernetes workload backing each agent at creation time. When provisioning is disabled (the default), create/delete only write the database row — no cluster is required.
