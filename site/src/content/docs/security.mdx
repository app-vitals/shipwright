...
no admin override), deferring to GitHub's branch protection rather than bypassing it. If a required
review isn't satisfied, the merge fails, and Shipwright releases its claim on the task and marks the
task/PR **blocked** so a human can add the missing approval.
