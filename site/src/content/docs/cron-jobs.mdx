...
`shipwright-deploy`) shows as enabled in the admin console but never has any runs in Cron Logs,
check whether `shipwright-loop` itself is enabled (see "Loop-driven vs standalone crons" above): a
reconciled pipeline cron is a child of `shipwright-loop` and never gets its own independent
schedule, so its `enabled` toggle only takes effect once `shipwright-loop` reads it and dispatches
that phase directly — there's no standalone tick to show up as `silent` or `skipped` in the
meantime.
