...
1. **Pass 1** creates or updates each manifest cron entry by name. If a matching row already
...
A final orphan pass deletes any existing system cron whose name is no longer present in the
manifest, so removed crons clean themselves up on the next reconcile.
...
Not every cron discovers its own work. The five pipeline crons — `shipwright-plan`,
`shipwright-dev-task`, `shipwright-review`, `shipwright-patch`, and `shipwright-deploy` — are children of
...
- None of the five commands scans for its own work. Each one responds `[silent]` and exits
...
|---|---|---|---|---|---|
| `shipwright-plan` | every minute | `/shipwright:plan-session` | — | off | Runs an autonomous planning session for the flagged PRD task `shipwright-loop` selects. Needs both this toggle and the `SHIPWRIGHT_AGENT_AUTONOMOUS_PLAN_SESSION_ENABLED` env var. Inert if `shipwright-loop` is disabled. |
...
| `shipwright-loop` | every minute | internal | — | off | Drives the five phases above — merges their candidates and dispatches exactly one winning item per phase, per tick, until the queue is dry. |
...
`shipwright-site-docs-freshness` isn't in the table above — its precheck and command are scoped to this repo's own marketing site rather than being useful for every agent's repo, so it's available to wire in per-agent as a custom, non-system cron rather than being seeded by default.
...
- **Phase** — for `shipwright-loop` runs, which phase (plan/dev-task/review/patch/deploy) it dispatched
...
|---|---|---|
| A cron run shows outcome `silent` in Cron Logs | Look at the run details — this is normal. The run completed successfully and simply had no qualifying work to report. | Most cron ticks find no work and end silently by design. This is the system working correctly, not a bug. |
| A cron run shows outcome `skipped` in Cron Logs | Look at the run's outcome in Cron Logs. A `skipped` outcome means the cron's `preCheck` script exited non-zero or produced empty output. The tick was suppressed *before* a Claude turn even started. | This is the expected shape for a cron with no work ready. The `preCheck` script found nothing to act on and correctly short-circuited the expensive Claude run. |
| A cron never appears in Cron Logs at all (no recent runs, no `silent`, no `skipped`) | Check the enabled/disabled badge on the cron's row in the agent's **Cron Jobs** card in the admin console. | If the badge shows **disabled**, the cron doesn't run at all — `node-cron` never scheduled it in the first place. Re-enable it if you want it to fire. |
| A pipeline cron (`shipwright-plan`, `shipwright-dev-task`, `shipwright-review`, `shipwright-patch`, or `shipwright-deploy`) shows as **enabled** but has no runs in Cron Logs | Check whether `shipwright-loop` itself is enabled in the agent's **Cron Jobs** card (see "Loop-driven vs standalone crons" section above). | A reconciled pipeline cron is a child of `shipwright-loop` and never gets its own independent schedule. Its `enabled` toggle only takes effect once `shipwright-loop` reads it and dispatches that phase directly. If `shipwright-loop` is disabled, the pipeline cron will not fire on its own — there's no standalone tick. Re-enable `shipwright-loop` to restore pipeline dispatch. |
