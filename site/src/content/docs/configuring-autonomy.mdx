...
> **Note:** New agents start with `shipwright-dev-task`, `shipwright-review`, and
> `shipwright-patch` all enabled (Stage 2). Disable `shipwright-review` and `shipwright-patch`
> via the admin UI to begin at Stage 1.
...
| `allow_self_review` | false | Review own PRs |
...
| `allow_self_review` | `false` | Allow the agent to review PRs it opened. Set `false` to require a human reviewer on agent-authored PRs. |
