...
| Resource | Endpoints |
|----------|-----------|
| Agents | `GET /agents`, `GET /agents/:id`, `PATCH /agents/:id`, `DELETE /agents/:id`, `POST /agents/:id/provision` |
...
There is no agent-creation endpoint on this JSON API — new agents are created exclusively
through the admin console's form at `/admin/agents/new`.
...
Ready to configure how much the agent does on its own? See
[Configuring Autonomy](/docs/configuring-autonomy) next, then
[Deploying to Cloud](/docs/deploying-to-cloud) when you're ready to ship.
