...
- **`state/dev-agent.env`** — credentials for the agent pane. On the first `task stack` run, if
  this file is missing it is auto-created from `state/dev-agent.env.example` and the command
