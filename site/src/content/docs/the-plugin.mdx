...
| Skill | What it does |
|-------|--------------|
| `triage-dependency-bot-pr` | Analyze a single Dependabot/Renovate PR — classify risk (merge/review/hold) and post a review comment directly by default (opt out via `auto_post_dependency_bot_triage: false` to stage instead) |
| `triage-dependency-bot-prs` | Scan all repos for open Dependabot/Renovate PRs, triage new ones, and post review comments in batch by default (opt out via `auto_post_dependency_bot_triage: false` to stage instead) |
