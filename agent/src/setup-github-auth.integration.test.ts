/**
 * agent/src/setup-github-auth.integration.test.ts
 *
...
describe("setupGitHubAuth — PAT path", () => {
  ...
  test("does not call startBackgroundRefresh on PAT path", async () => {
    const spawnSync = makeSpawnSync();
    const { startBackgroundRefresh } = makeTokenManager();
    const createTokenManager = mock(() => {
      throw new Error("should not be called");
    });
    ...
    expect(startBackgroundRefresh).not.toHaveBeenCalled();
  });
});
