...
describe("reconcileDeployingTasks", () => {
  ...
  test("a single task's workflow-run lookup failure does not abort reconciliation of the rest of the batch", async () => {
    ...
    const { deps, taskPatchCalls } = makeDeps({
      repos: ["acme/example-repo", "other-org/other-repo"],
  ...
  test("a deploying task whose resolveTaskRepo() is out of scope is skipped with zero gh calls, while an in-scope task in the same batch still reconciles", async () => {
    const taskInScope: PrOpenTaskRecord = {
      id: "task-deploying-in-scope",
      repo: "acme/repo-a",
      mergedAt: "2026-07-17T00:00:00.000Z",
    };
    const taskOutOfScope: PrOpenTaskRecord = {
      id: "task-deploying-out-of-scope",
      repo: "acme/repo-b",
      mergedAt: "2026-07-17T00:00:00.000Z",
    };
    const ghCalls: string[] = [];
    const { deps, taskPatchCalls } = makeDeps({
      repos: ["acme/repo-a", "acme/repo-b"],
      deployingTasks: [taskInScope, taskOutOfScope],
      workflowRunsResults: {
        "acme/repo-a": [
          {
            id: 1,
    ...
    const originalGhListWorkflowRuns = deps.ghListWorkflowRuns;
    deps.ghListWorkflowRuns = async (
      repo: string,
      workflow: string,
      limit: number,
    ) => {
      ghCalls.push(repo);
      return await originalGhListWorkflowRuns(repo, workflow, limit);
    };

    await reconcileDeployingTasks(deps);

    expect(ghCalls).toEqual(["acme/repo-a"]);
    expect(taskPatchCalls).toHaveLength(1);
    expect(taskPatchCalls[0].id).toBe("task-deploying-in-scope");
  });
});
